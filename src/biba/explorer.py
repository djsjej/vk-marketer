...
ENDPOINTS_TO_EXPLORE: list[tuple[str, str, str]] = [
    # ====================================================================
    # БАЛАНС / АККАУНТ (поиск рабочего эндпоинта баланса — 26.06.2026)
    # Бот не видит баланс: /agency/clients.json не работает у прямого
    # кабинета. Биба проверяет кандидатов, по ответу впишем точный путь
    # и поле в get_balance() — без угадывания.
    # ====================================================================
    ("budget", "/budget.json", "Баланс/бюджет кабинета (кандидат)"),
    ("user", "/user.json", "Инфо пользователя/аккаунта, возможно с балансом (кандидат)"),
    ("account", "/account.json", "Аккаунт кабинета (кандидат)"),
    ("clients", "/clients.json", "Клиенты/кабинеты (кандидат)"),
